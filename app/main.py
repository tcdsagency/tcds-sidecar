...
async def send_agencyzoom_sms(request: SMSRequest):
    """
    Send SMS via AgencyZoom using HTTP with session cookies.
    Uses cached session if available, otherwise extracts fresh session.
    """
    try:
        ...
        if cached and cached.get("cookies"):
            ...
            print("[SMS] Using cached session from token_cache")
